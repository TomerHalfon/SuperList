 ...
 * This script creates the initial JSON files from mock data
 */

const fs = require('fs').promises;
const path = require('path');

// Mock data (copied from src/lib/data/mock-data.ts)
const mockItems = {
  "items": [
...
async function initializeStorage() {
  try {
    ...
    await fs.writeFile(itemsPath, JSON.stringify(mockItems, null, 2), 'utf-8');
    console.log('✅ Created items.json with', mockItems.items.length, 'items');
    
    // Write lists.json
    const listsPath = path.join(dataDir, 'lists.json');
    await fs.writeFile(listsPath, JSON.stringify(mockShoppingLists, null, 2), 'utf-8');
    console.log('✅ Created lists.json with', mockShoppingLists.lists.length, 'shopping lists');
